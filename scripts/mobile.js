...
SA.redirection_mobile = function (configuration) {
  ...
  var agent = navigator.userAgent.toLowerCase(),
    ...
    mobile_host =
      ...
      mobile_prefix +
        "." +
        (!!host.match(/^www\./i) ? host.substring(4) : host),
  ...
  if (
    !!agent.match(
      /(iPad|SCH-I|xoom|NOOK|silk|kindle|GT-P|touchpad|kindle|sch-t|viewpad|bolt|playbook|Nexus 7)/i
    )
  ) {
    // Check if the redirection needs to happen for tablets
    isUATablet =
      config.tablet_redirection === TRUE || !!config.tablet_host ? true : false;
    isUAMobile = false;
  }
  ...
  if ((isUATablet || isUAMobile) && !(isCookieSet || isSessionStorage)) {
    ...
    if (isUATablet) {
      document.location.href =
        mobile_protocol + "//" + tablet_host + path_query;
    } else if (isUAMobile) {
      document.location.href =
        mobile_protocol + "//" + mobile_host + path_query;
    }
  }
};
